...
class ReverseDenoisingSlide(Scene, DDPMBaseMixin):
    def construct(self):
        ...
        # Title
        title = Text("Reverse Process: Learned Denoising", 
                    font="TeX Gyre Termes", font_size=36, color="#90EE90")
        title.to_edge(UP, buff=0.8)
        self.play(Write(title))
        
        # Start with pure noise
        noise_img = self.create_noisy_image(1.0, size=1.2)
        ...
        for i, (pos, noise) in enumerate(zip(positions, noise_levels)):
            if noise == 0.0:
                ...
                clean_img = self.create_noisy_image(noise, size=1.2)
            ...
            nn_arrow = Arrow(nn_box.get_bottom(), clean_img.get_top(),
                           color="#90EE90", stroke_width=3)
