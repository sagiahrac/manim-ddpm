...
class DDPMBaseMixin:
    ...
    def create_image_representation(self, color, opacity=0.8, size=1.0):
        ...
        for i in range(3):
            for j in range(3):
                ...
                small_square.set_fill(
                    color=interpolate_color(color, WHITE, 0.3 * np.random.random())
                )
    ...
    def tmp_create_noisy_image(self, noise_level, size=1.0):
