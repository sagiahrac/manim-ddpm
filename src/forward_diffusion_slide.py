...
class ForwardDiffusionSlide(Scene, DDPMBaseMixin):
    def construct(self):
        ...
        title = Text("Forward Process: Adding Noise", 
                    font="TeX Gyre Termes", font_size=36, color="#FFD700")
        ...
        original = self.create_image_representation(BLUE_B, opacity=0.9, size=image_size)
        ...
        for i, (pos, noise) in enumerate(zip(positions, noise_levels)):
            ...
            noisy_img = self.create_noisy_image(noise, size=image_size)
        ...
        # Add explanation box
        explanation = Text("Each step adds Gaussian noise", 
                         font_size=20, color="#B0B0B0")
