...
class ProcessOverviewSlide(Scene, DDPMBaseMixin):
    def construct(self):
        ...
        forward_label = Text(
            "Forward Process: Adding Noise", font_size=26, color="#FF6B6B", weight=BOLD
        )
        ...
        for i, (pos, noise_level, math_label) in enumerate(
            zip(positions, noise_levels, math_labels)
        ):
            ...
            try:
                ...
                base_img.scale(0.1)
                base_img.set_opacity(
                    1 - noise_level * 0.8
                )  # Fade out as noise increases
            except:
                base_img = Square(
                    side_length=0.6,
                    fill_color=ORANGE,
            ...
            for _ in range(num_noise_dots):
                ...
                img_width = (
                    base_img.width * 0.8
                )  # Use 80% of image width for noise area
                img_height = (
                    base_img.height * 0.8
                )  # Use 80% of image height for noise area

                noise_dot = Dot(
                    point=[
                        img_center[0]
                        + np.random.uniform(-img_width / 2, img_width / 2),
                        img_center[1]
                        + np.random.uniform(-img_height / 2, img_height / 2),
                        0,
                    ],
            ...
            if i < len(noise_levels) - 1:
                transition_label = MathTex(
                    r"q(x_{t+1}|x_t)", font_size=16, color="#FF6B6B"
                )
            else:
                transition_label = MathTex(
                    r"q(x_T|x_{T-1})", font_size=16, color="#FF6B6B"
                )
            ...
            self.play(FadeIn(noisy_group, scale=0.8), Write(step_label), run_time=0.8)

            # Add arrow and transition notation after the image (so they appear on top)
            self.play(Create(arrow), Write(transition_label), run_time=0.6)

            images.append(noisy_group)
            self.wait(0.3)

        # Final noise label (DDPM style)
        pure_noise_label = MathTex(
            r"x_T \sim \mathcal{N}(0, I)", font_size=20, color="#FF6B6B"
        )
